...
require_once File::build_path(Array("Utilisateur.php"));
...
class Trajet {
    ...
    public static function findPassagers($id){
        $sql = "SELECT login, nom, prenom FROM passager p JOIN utilisateur u ON p.utilisateur_login = u.login WHERE trajet_id = :id" ;
        ...
        $rep_prep->setFetchMode(PDO::FETCH_CLASS, 'Utilisateur');

        $rep_prep->execute($values);

        return $rep_prep->fetchAll();
